...
2. **Add Design Sources:** Add the top-level file and all submodules:
   ...
   * `output_counter.vhd`
   * `argmax_unit.vhd`
3. **Add Simulation Sources:** Add the testbench file: `snn_tb.vhd`.
4. **Add Constraints:** Add the constraints file: `ac701_snn_constraints.xdc`.
5. **Update Memory Paths:** Open `snn_top_level.vhd` and `snn_tb.vhd` in the Vivado text editor. Locate the `generic` parameters defining the file paths for the weights and spikes (e.g., `HIDDEN_WEIGHTS_FILE_PATH`). Update these absolute paths to point to the `.mem` files generated by the `ANN_to_SNN.py` script on your local machine.
