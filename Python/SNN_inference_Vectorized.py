...
hidden_potentials = np.zeros(w_h_32.shape[0], dtype=np.int32) # 64
output_potentials = np.zeros(w_o_32.shape[0], dtype=np.int32) # 10
hidden_spikes = np.zeros(w_h_32.shape[0], dtype=np.int32) # 64
output_spike_counts = np.zeros(w_o_32.shape[0], dtype=np.int32) # 10
...
spike_patterns_32 = spike_patterns.astype(np.int32)
...
for repeat in range(num_repeats):
    ...
    for i in range(num_samples):
        label = labels[i]  # Get the true label for the current sample
        ...
        for t in range(T):
            ...
            input_spikes = spike_patterns_32[i, t, :] # 784
            
            # --- HIDDEN LAYER ---
            # Matrix multiplication replaces the nested i and j weight loops
            I_syn_hidden = input_spikes @ w_h_32.T # [1, 64] = [1, 784] @ [784, 64]
            ...
            output_spike_counts += output_spikes # [1, 10] = [1, 10] + [1, 10]
            
            output_potentials[output_spikes == 1] = 0

        # --- VECTORIZED READOUT & TIE-BREAKER ---
        # Sum the total spikes for each sample to find dead-locks
        total_output_spikes = np.sum(output_spike_counts)

        # Find the argmax for both conditions across the whole batch
        pred_potential = np.argmax(output_potentials)
        pred_spike = np.argmax(output_spike_counts)

        # If total spikes == 0, use potentials, otherwise use spikes
        predicted_label = np.where(total_output_spikes == 0, pred_potential, pred_spike)
